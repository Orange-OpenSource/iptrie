...
fn parse_errors() {
    assert_eq!(
        "1::/12".parse::<Ipv4Prefix>(),
        Err(IpPrefixError::AddrParseError)
    );
    assert_eq!(
        "1.1.1.1/12".parse::<Ipv6NetPrefix>(),
        Err(IpPrefixError::AddrParseError)
    );
}
...
#[test]
fn prefix_ipv6_128_trunc() {
    prefix_ipv6_trunc::<Ipv6Prefix>()
}
#[test]
fn prefix_ipv6_64_trunc() {
    prefix_ipv6_trunc::<Ipv6NetPrefix>()
}
