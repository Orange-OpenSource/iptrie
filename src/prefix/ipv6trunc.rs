...
impl IpPrefixCovering<Ipv6Prefix> for Ipv6Prefix56 {
    ...
    {
        if ((other.bitslot_trunc() >> 64) as u64) & self.bitmask() != self.bitslot_trunc() {
...
impl IpPrefixCovering<Ipv6Prefix56> for Ipv6Prefix {
    ...
    {
        if u128::from(other.network()) & self.bitmask() != self.bitslot_trunc() {
            IpPrefixCoverage::NoCoverage
        } else {
            match self.len().cmp(&other.len()) {
                Ordering::Less => IpPrefixCoverage::WiderRange,
...
impl IpPrefixCovering<Ipv6Prefix> for Ipv6Prefix120 {
    #[inline]
    fn covering(&self, other: &Ipv6Prefix) -> IpPrefixCoverage {
        if other.bitslot_trunc()  & self.bitmask() != self.bitslot_trunc() {
            IpPrefixCoverage::NoCoverage
        } else {
            match self.len().cmp(&other.len()) {
                Ordering::Less => IpPrefixCoverage::WiderRange,
...
impl IpPrefixCovering<Ipv6Net> for Ipv6Prefix56 {
    ...
    fn covering(&self, other: &Ipv6Net) -> IpPrefixCoverage {
        if ((other.bitslot_trunc() >> 64) as u64) & self.bitmask() != self.bitslot_trunc() {
            dbg!(IpPrefixCoverage::NoCoverage)
        } else {
            match self.len().cmp(&other.len()) {
                Ordering::Less => IpPrefixCoverage::WiderRange,
                Ordering::Equal => IpPrefixCoverage::SameRange,
                Ordering::Greater => dbg!(IpPrefixCoverage::NoCoverage)
            }
        }
    }
}
